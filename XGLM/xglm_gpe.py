...
languages = ["en", "de", "ar", "ru"]  # English, German, Arabic, Russian
...
# Function to tokenize using GPE + BPE
def tokenize_with_gpe(texts, tokenizer):
    """
    Tokenizes a list of texts using Grapheme Pair Encoding (GPE) followed by BPE-based tokenization.
...
df_compression.to_csv("xglm_gpe_max_compression_ratio.csv", index=False)
print("Results saved to xglm_gpe_max_compression_ratio.csv")
...
df_parity.to_csv("xglm_gpe_min_tokenization_parity.csv", index=False)
print("Results saved to xglm_gpe_min_tokenization_parity.csv")
print(df_parity)
